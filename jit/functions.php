...
add_action('init', function () {
    ...
    // --- Product Country: 原産国 ---
    register_taxonomy('product_country', ['product'], [
      'labels' => [
        'name'          => '原産国',
        'singular_name' => '原産国',
        'search_items'  => '原産国を検索',
  ...
  add_action('product_country_edit_form_fields', function ($term) {
    ...
    <tr class="form-field term-flag-wrap">
      ...
      <td>
        <select name="country_flag" id="country_flag">
          <?php foreach ($flags as $value => $label): ?>
            <option value="<?php echo esc_attr($value); ?>" <?php selected($flag, $value); ?>>
  ...
  add_action('created_product_country', function ($term_id) {
    if (isset($_POST['country_flag'])) {
      update_term_meta($term_id, 'country_flag', sanitize_text_field($_POST['country_flag']));
    }
  });
  ...
  add_filter('manage_product_country_custom_column', function ($content, $column_name, $term_id) {
    if ($column_name === 'flag') {
      $flag = get_term_meta($term_id, 'country_flag', true);
      if ($flag) {
        return '<span style="font-size: 1.5em;">' . esc_html($flag) . '</span>';
      }
    }
    return $content;
  }, 10, 3);
  ...
  add_action('restrict_manage_posts', function ($post_type) {
    ...
    // 商品カテゴリフィルター
    $selected_category = isset($_GET['product_category']) ? $_GET['product_category'] : '';
    ...
    // 原産国フィルター
    $selected_country = isset($_GET['product_country']) ? $_GET['product_country'] : '';
    $countries = get_terms([
  ...
  add_action('news_category_edit_form_fields', function ($term) {
    ...
    <tr class="form-field term-order-wrap">
      ...
      <td>
        <input type="number" name="news_category_order" id="news_category_order" value="<?php echo esc_attr($order); ?>" min="0" step="1">
        <p class="description">数値が小さいほど先に表示されます（0が最初）。</p>
      </td>
    </tr>
    <?php
  ...
  add_action('created_news_category', function ($term_id) {
    if (isset($_POST['news_category_order'])) {
      update_term_meta($term_id, 'news_category_order', intval($_POST['news_category_order']));
    }
  });
  ...
  add_filter('manage_news_category_custom_column', function ($content, $column_name, $term_id) {
  ...
  add_action('parse_term_query', function ($query) {
    if (!is_admin()) return;
    if (!isset($_GET['orderby']) || $_GET['orderby'] !== 'order') return;
  ...
  add_action('quick_edit_custom_box', function ($column_name, $screen, $name) {
    if ($name !== 'news_category' || $column_name !== 'order') return;
    ?>
    <fieldset>
  ...
  add_action('admin_footer', function () {
    ...
    if (!$screen || $screen->taxonomy !== 'news_category') return;
    ?>
    <script type="text/javascript">
  ...
  add_action('pre_get_posts', function ($q) {
    ...
    // news のアーカイブだけ対象
    if (!$q->is_post_type_archive('news')) return;
